...
router.get('/', async (req, res) => {
  ...
  const faqs = await db.prepare(sql).all(...params);
  res.json({ faqs });
});
...
router.post('/', authMiddleware, async (req, res) => {
  ...
  const faq = await db.prepare('SELECT * FROM faqs WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', authMiddleware, async (req, res) => {
  const db = getDb();
  const existing = await db.prepare('SELECT id FROM faqs WHERE id = ?').get(req.params.id);
