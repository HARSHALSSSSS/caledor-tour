...
window.CmsSchema = (() => {
  ...
  function imageField(target, label, value = "", options = {}) {
    ...
    const bg = value ? `background-image:url('${esc(value)}')` : "";
  ...
  function testimonialItemRow(item = {}) {
    return `<div class="team-card" data-list="testimonial-items">
      <div class="team-fields"><div class="form-grid">
        <div class="field-full"><label>Quote</label><textarea name="quote">${esc(item.quote || "")}</textarea></div>
        <div class="field"><label>Name</label><input name="name" value="${esc(item.name || "")}" /></div>
        <div class="field"><label>Role / Company</label><input name="role" value="${esc(item.role || "")}" /></div>
